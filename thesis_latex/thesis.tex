...
\begin{itemize}
	...
	\verb|provider| is used to send transactions, it also contains the signer, which is needed to sign transactions. For instance, actions that only read from the blockchain will only require the provider, and transactions that imply a state change will require to sign the transaction: \verb|provider.getSigner()|
\end{itemize}
Since this application is using metamask, the Infura provider will be used to interact with users and the network.

\subsection{Web3 Library: ethers}
...
\begin{figure}[htp]
	\centering
	\includegraphics[width=1.01\textwidth]{./images/sec1}
	\caption{Deposit interaction}
	\label{fig:deposit_seq}
\end{figure}
Using ethers, the interaction with smart contracts, previously tested with brownie, can be implemented. This enables users' accounts, to interact with the Aave protocol through the User Interface components, which triggers the methods implemented using ethers. For example, the interaction between the application methods and the smart contracts, for depositing DAI can be seen in~\ref{fig:deposit_seq}
\subsubsection{User or Account Info}
%%%%%
%%%%% Chapter
